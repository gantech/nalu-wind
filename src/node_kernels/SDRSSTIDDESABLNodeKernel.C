...
SDRSSTIDDESABLNodeKernel::SDRSSTIDDESABLNodeKernel(const stk::mesh::MetaData& meta)
  ...
  const stk::mesh::FastMeshIndex& node)
{
  ...
  const DblType eddyLengthRANS = stk::math::sqrt(tke) /
      stk::math::max(betaStar_ * sdr, small);
  ...
  const DblType Dk = density * stk::math::sqrt(tke) * tke /
      stk::math::max(eddyLengthDES, small);

  const DblType f_des_abl = 0.5*stk::math::tanh(
      (abl_bndtw_ - dw)/abl_deltandtw_) + 0.5;
