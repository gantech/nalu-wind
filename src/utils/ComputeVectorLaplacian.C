// Copyright 2017 National Technology & Engineering Solutions of Sandia, LLC
// (NTESS), National Renewable Energy Laboratory, University of Texas Austin,
// Northwest Research Associates. Under the terms of Contract DE-NA0003525
// with NTESS, the U.S. Government retains certain rights in this software.
//
// This software is released under the BSD 3-clause license. See LICENSE file
// for more details.
//


#include "utils/ComputeVectorLaplacian.h"

#include <master_element/MasterElement.h>
#include "master_element/MasterElementFactory.h"

#include <stk_mesh/base/Part.hpp>
#include <stk_mesh/base/Entity.hpp>
#include <stk_mesh/base/MetaData.hpp>
#include <stk_mesh/base/BulkData.hpp>
#include <stk_mesh/base/Field.hpp>
#include <stk_mesh/base/FieldParallel.hpp>
#include <stk_mesh/base/FieldBLAS.hpp>
#include <stk_mesh/base/GetBuckets.hpp>

namespace sierra {
namespace nalu {

void compute_vector_laplacian(
  stk::mesh::BulkData& bulk,
  stk::mesh::PartVector& partVec,
  stk::mesh::PartVector& bndyPartVec,
  stk::mesh::FieldBase* vectorField,
  stk::mesh::FieldBase* gradVectorField,
  stk::mesh::FieldBase* laplacianField,
  const bool hasMeshDeformation )
{
  stk::mesh::MetaData& meta = bulk.mesh_meta_data();
  const int nDim = meta.spatial_dimension();

  const std::string coordName = hasMeshDeformation ? "current_coordinates" : "coordinates";
  VectorFieldType* coordinates =
    meta.get_field<VectorFieldType>(stk::topology::NODE_RANK, coordName);
  ScalarFieldType* dualVol =
    meta.get_field<ScalarFieldType>(stk::topology::NODE_RANK, "dual_nodal_volume");

  std::vector<double> wsCoordinates; // Need this to get scs area
  std::vector<double> wsVector; // Need this to compute derivative
  std::vector<double> wsScsArea; // Need this to store computed scs area

  std::vector<double> ws_dndx; // What is the difference between the two?
  std::vector<double> ws_deriv;
  std::vector<double> ws_det_j; // What is this?

  std::vector<double> gradVIp(nDim*nDim,0.0); //Gradient of vector at ip
  std::vector<double> snGradVIp(nDim,0.0); // Surface-normal gradient of vector at ip
  double scs_error = 0.0;

  stk::mesh::Selector sel = ( meta.locally_owned_part() | meta.globally_shared_part() )
                          & stk::mesh::selectUnion(partVec);
  const auto& bkts =
      bulk.get_buckets( stk::topology::ELEMENT_RANK, sel );

  // reset laplacian field
  stk::mesh::field_fill(0.0, *laplacianField, sel);

  for (auto b: bkts) {
    MasterElement* meSCS =
        MasterElementRepo::get_surface_master_element(b->topology());

    const int nodesPerElement = meSCS->nodesPerElement_;
    const int numScsIp = meSCS->num_integration_points();
    const int* lrscv = meSCS->adjacentNodes();

    wsCoordinates.resize(nodesPerElement*nDim);
    wsVector.resize(nodesPerElement*nDim);
    wsScsArea.resize(numScsIp*nDim);

    ws_dndx.resize(nDim*numScsIp*nodesPerElement);
    ws_deriv.resize(nDim*numScsIp*nodesPerElement);
    ws_det_j.resize(numScsIp);
    
    size_t length = b->size();
    for ( size_t k = 0 ; k < length ; ++k ) {

      const stk::mesh::Entity* node_rels = b->begin_nodes(k);
      int num_nodes = b->num_nodes(k);
      for ( int ni = 0; ni < num_nodes; ++ni ) {
        stk::mesh::Entity node = node_rels[ni];
        const double* coords =  stk::mesh::field_data(*coordinates, node);
        const double* mv =  (double*)stk::mesh::field_data(*vectorField, node);
        for (int iDim=0; iDim < nDim; iDim++) {
          wsCoordinates[ni*nDim+iDim] = coords[iDim];
          wsVector[ni*nDim+iDim] = mv[iDim];
        }
      }

      // compute geometry
      double scs_error = 0.0;
      meSCS->determinant(1, wsCoordinates.data(), wsScsArea.data(), &scs_error);
      meSCS->grad_op(1, wsCoordinates.data(), ws_dndx.data(), ws_deriv.data(), ws_det_j.data(), &scs_error);

      for ( int ip = 0; ip < numScsIp; ++ip ) {

        // Get left and right nodes for this IP
        // Get gradient at this IP
        // Do dot product of gradient and face area vector to get face normal derivative
        // Assemble to laplacian at left and right nodes
        
        const int ipNdim = ip*nDim;
        const int offSetSF = ip*nodesPerElement;

        // left and right nodes for this ip
        const int il = lrscv[2*ip];
        const int ir = lrscv[2*ip+1];

        stk::mesh::Entity nodeL = node_rels[il];
        stk::mesh::Entity nodeR = node_rels[ir];

        // pointer to fields to assemble
        double* laplL = (double*)stk::mesh::field_data(*laplacianField, nodeL);
        double* laplR = (double*)stk::mesh::field_data(*laplacianField, nodeR);

        double* dualVolL = stk::mesh::field_data(*dualVol, nodeL);
        double* dualVolR = stk::mesh::field_data(*dualVol, nodeR);

        //Compute derivative of vector field  at this ip - 9 components
        //First zero out the derivative
        for ( int j = 0; j < nDim; ++j ) {
          for ( int k = 0; k < nDim; ++k)
            gradVIp[j*nDim+k] = 0.0;
        }

        for ( int ic = 0; ic < nodesPerElement; ++ic ) {
          const int icNdim = ic*nDim;
          const int offSetDnDx = nDim*nodesPerElement*ip + icNdim;
          for ( int j=0; j < nDim; j++) {            
            for (int k=0; k < nDim; k++) {
              const double r = ws_dndx[offSetDnDx+j];
              gradVIp[j*nDim+k] += r * wsVector[ic*nDim+j];
            }
          }
        }

        //Compute dot product with area
        for (int j=0; j < nDim; j++) {
          snGradVIp[j] = 0.0;
          for (int k=0; k < nDim; k++)
            snGradVIp[j] += gradVIp[j*nDim+k] * wsScsArea[ipNdim+k];
          laplL[j] += snGradVIp[j] / (*dualVolL);
          laplR[j] -= snGradVIp[j] / (*dualVolR);
        }

      }
    }
  }

  // sum up interior divergence values and return if boundary part not specified
  if(bndyPartVec.size() == 0) {
    stk::mesh::parallel_sum(bulk, {laplacianField});
    return;
  }

  std::vector<double> ws_shape_function;
  std::vector<double> wsGradVector;  
  GenericFieldType* exposedAreaVec =
    meta.get_field<GenericFieldType>(meta.side_rank(), "exposed_area_vector");
  // stk::mesh::Selector face_sel = meta.locally_owned_part()
  //                               & stk::mesh::selectUnion(bndyPartVec);
  stk::mesh::Selector face_sel = meta.locally_owned_part() & stk::mesh::selectField(*exposedAreaVec);
  const auto& face_bkts =
      bulk.get_buckets( meta.side_rank(), face_sel );
  for (auto b: face_bkts) {
    // extract master element
    MasterElement* meFC =
        MasterElementRepo::get_surface_master_element(b->topology());

    const int nodesPerFace = meFC->nodesPerElement_;
    const int numScsIp = meFC->num_integration_points();
    const int* ipNodeMap = meFC->ipNodeMap();

    wsGradVector.resize(nodesPerFace*nDim*nDim);
    ws_shape_function.resize(numScsIp*nodesPerFace);
    meFC->shape_fcn(ws_shape_function.data());

    size_t length = b->size();
    for ( stk::mesh::Bucket::size_type k = 0 ; k < length ; ++k ) {
        
      // face data
        const double* areaVec = (double*)stk::mesh::field_data(*exposedAreaVec, *b, k);
      const stk::mesh::Entity* face_node_rels = b->begin_nodes(k);
      int num_nodes = b->num_nodes(k);
      // sanity check on num nodes
      ThrowAssert( num_nodes == nodesPerFace );
      //Gather grad vector field on all face nodes
      for ( int ni = 0; ni < num_nodes; ++ni ) {
        stk::mesh::Entity node = face_node_rels[ni];
        const double* gv = (double*)stk::mesh::field_data(*gradVectorField, node);
        for ( int j=0; j < nDim; j++) {
          for (int k=0; k < nDim; k++) 
            wsGradVector[ni*nDim*nDim+j*nDim+k] = gv[j*nDim+k];
        }
      }

      // start assembly
      for ( int ip = 0; ip < numScsIp; ++ip ) {
        // nearest node
        const int nn = ipNodeMap[ip];
        stk::mesh::Entity nodeNN = face_node_rels[nn];
        double* laplV = (double*)stk::mesh::field_data(*laplacianField, nodeNN);
        double* volNN = (double*)stk::mesh::field_data(*dualVol, nodeNN);

        // interpolate to scs point; operate on saved off grad vector field
        // First set gradVIp to zero
        for ( int j = 0; j < nDim; ++j ) {
          for ( int k = 0; k < nDim; ++k) {
            gradVIp[j*nDim+k] = 0.0;
          }
        }

        const int offSet = ip*nodesPerFace;
        for ( int ic = 0; ic < nodesPerFace; ++ic ) {
          for (int j=0; j < nDim; j++) {
            for (int k=0; k < nDim; k++) {
                gradVIp[j*nDim+k] += ws_shape_function[offSet+ic]
                    * wsGradVector[ic*nDim*nDim+j*nDim+k];
            }
          }
        }

        //Compute dot product with area
        for ( int j = 0; j < nDim; ++j ) {
            snGradVIp[j] = 0.0;
          for ( int k = 0; k < nDim; k++)
              snGradVIp[j] += gradVIp[j*nDim+k] * areaVec[ip*nDim+k];
          laplV[j] += snGradVIp[j]/(*volNN);
        }
      }
          
    }
  }
  // parallel sum the divergence across all processors
  stk::mesh::parallel_sum(bulk, {laplacianField});
  return;
}

}
}
