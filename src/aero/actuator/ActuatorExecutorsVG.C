...
ActuatorLineVGNGP::operator()()
{
  ...
  Kokkos::parallel_for(
    "updatePointLocationsActuatorNgpVG", localRangePolicy,
    ActVGUpdatePoints(actBulk_, nPts, p1, p2));
  ...
  Kokkos::parallel_for(
    "interpolateVelocitiesActuatorNgpVG", numActPoints_,
  ...
  Kokkos::parallel_for(
    "assignVGVelActuatorNgpVG", localRangePolicy,
    ActVGAssignVel(actBulk_));

  ActVGComputeForce(actBulk_, actMeta_);

  compute_fllc();

  ActVGWriteToFile(actBulk_, actMeta_);
  ...
  if (useSpreadActuatorForce_) {
    Kokkos::parallel_for(
      "spreadForcesActuatorNgpVG", localSizeCoarseSearch,
      SpreadActuatorForce(actBulk_, stkBulk_));
  } else {
    // --  use ActVGSpreadForceWhProjection
    Kokkos::parallel_for(
      "spreadForceUsingProjDistance", localSizeCoarseSearch,
      ActVGSpreadForceWhProjection(actBulk_, stkBulk_));
  }
