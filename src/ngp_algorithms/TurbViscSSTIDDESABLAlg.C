...
TurbViscSSTIDDESABLAlg::TurbViscSSTIDDESABLAlg(
  ...
  : Algorithm(realm, part),
    ...
    dualNodalVolume_(get_field_ordinal(realm.meta_data(),
                                       "dual_nodal_volume")),
...
TurbViscSSTIDDESABLAlg::execute()
{
  ...
  const DblType cmuEps = realm_.get_turb_model_constant(TM_cmuEps);
  const DblType bndtw = realm_.get_turb_model_constant(TM_abl_bndtw);
  const DblType deltandtw = realm_.get_turb_model_constant(TM_abl_deltandtw);
  const int nDim = meta.spatial_dimension();

  nalu_ngp::run_entity_algorithm(
    "TurbViscSSTIDDESABLAlg", ngpMesh, stk::topology::NODE_RANK, sel,
    KOKKOS_LAMBDA(const Traits::MeshIndex& meshIdx) {
      ...
      const DblType tvisc_abl = cmuEps*density.get(meshIdx, 0) *
          std::sqrt(tke.get(meshIdx, 0))*filter;

      const DblType f_des_abl = 0.5*stk::math::tanh(
          (bndtw - minD.get(meshIdx,0))/deltandtw) + 0.5;
      tvisc.get(meshIdx, 0) = f_des_abl * tvisc_sst + (1.0 - f_des_abl)
          * tvisc_abl;
      sdr.get(meshIdx,0) = f_des_abl * sdr.get(meshIdx,0) + (1.0 - f_des_abl)
          * std::sqrt(tke.get(meshIdx, 0)) / (cmuEps*filter);
